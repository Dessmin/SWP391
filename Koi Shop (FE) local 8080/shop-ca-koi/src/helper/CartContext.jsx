...
export const CartProvider = ({ children }) => {
  ...
  useEffect(() => {
    if (user && user.userId) {
      const sessionCart = getCartFromSession(user.userId); // Lấy giỏ hàng theo userId
  ...
  const addToCart = (product) => {
    ...
    saveCartToSession(user.userId, updatedCart); // Lưu giỏ hàng theo userId vào session
  };
  ...
  const removeFromCart = (productId, productType) => {
    ...
    setCartItems(updatedCart);
    saveCartToSession(user.userId, updatedCart); // Cập nhật giỏ hàng theo userId
  };
